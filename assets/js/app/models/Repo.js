define(['jquery', 'underscore', 'backbone', 'app/collections/Stargazers'], function($, _, Backbone, Stargazers) {
	...
	var Repo = Backbone.Model.extend({
		initialize: function() {
			...
			stargazers.url = this.get('stargazers_url') + "?per_page=36&page=" + ((Math.ceil(Math.random()*(stargazers_count / 36))) + 1);
			...
			forks.url = this.get('forks_url') + "?per_page=36&page=" + ((Math.ceil(Math.random()*(forks_count / 36))) + 1);
